...
export default function EditarInstalacionScreen() {
  const router = useRouter();
  const [nombre, setNombre] = useState('Consultorio 101');
  const [tipo, setTipo] = useState('Consultorio');
  const [ubicacion, setUbicacion] = useState('Piso 1, Torre A');

  const handleActualizar = () => {
    console.log('Actualizar:', { nombre, tipo, ubicacion });
    router.back();
  };

  const confirmarEliminacion = () => {
    Alert.alert("Eliminar Instalación", "¿Seguro que deseas eliminar este registro?", [
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.pageTitle}>Editar Instalación</Text>
      ...
      <TouchableOpacity style={styles.saveButton} onPress={handleActualizar}>
        <Text style={styles.saveButtonText}>Actualizar Instalación</Text>
      </TouchableOpacity>

      <TouchableOpacity style={styles.deleteButton} onPress={confirmarEliminacion}>
        <Text style={styles.deleteButtonText}>Eliminar Instalación</Text>
      </TouchableOpacity>
